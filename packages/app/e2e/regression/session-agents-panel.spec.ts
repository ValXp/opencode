...
const coordinatorID = "ses_agents_coordinator"
const nestedChildID = "ses_agents_nested_child"
const siblingID = "ses_agents_sibling"
const rootTitle = "Agents root session"
const childTitle = "Focused child agent"
const otherChildTitle = "Agent from another session"
const nestedChildTitle = "Nested child agent"
const siblingTitle = "Sibling agent"
...
test("keeps the agents provider mounted before the workspace tab opens", async ({ page }) => {
  ...
  await setup(page, {
    ...
    agentRunOverview: () => {
      overviewRequests++
      return { body: overview(run({ state: { type: "running" }, version: 1 })) }
    },
  })
...
test("repairs the snapshot after a locationless global reconnect event", async ({ page }) => {
  ...
  await setup(page, {
    ...
    agentRunOverview: () => {
      ...
      return {
        body:
          overviewRequests === 1 ? overview(run({ state: { type: "running" }, version: 1 })) : overview(),
      }
    },
  })
  await page.route(
...
test("recovers the global indicator after an overview failure while the session snapshot remains available", async ({
  ...
  await setup(page, {
    ...
    agentRunOverview: () => {
      requests++
      if (requests === 1) return { body: { error: "temporarily unavailable" }, status: 503 }
      return { body: overview(run({ state: { type: "running" }, version: 1 }), other) }
    },
  })
...
test("retries lineage and loads the complete session tree for a direct legacy nested-child route", async ({ page }) => {
  const requested: string[] = []
  let lineageFailed = false
  const nested = run({
    id: "arun_agents_nested_child",
    sessionID: nestedChildID,
    callerSessionID: coordinatorID,
    state: { type: "running" },
    version: 1,
  })
  const sibling = run({
    id: "arun_agents_sibling",
    sessionID: siblingID,
  ...
  const rootSnapshot = {
    ...
    nodes: [
      node(coordinatorID, rootID, "Coordinator agent", 1_700_000_001_500),
      node(nestedChildID, coordinatorID, nestedChildTitle, 1_700_000_002_000),
      node(siblingID, rootID, siblingTitle, 1_700_000_002_500),
    ],
    active: [nested, sibling],
    history: [],
  }
  await setup(page, {
    agentRun: (sessionID) => {
      requested.push(sessionID)
      return { body: rootSnapshot }
    },
    agentRunOverview: () => ({ body: { ...rootSnapshot, rootSessionID: undefined } }),
    sessions: [
      session(coordinatorID, "Coordinator agent", 1_700_000_001_500, { parentID: rootID }),
      session(nestedChildID, nestedChildTitle, 1_700_000_002_000, { parentID: coordinatorID }),
      session(siblingID, siblingTitle, 1_700_000_002_500, { parentID: rootID }),
    ],
    newLayoutDesigns: false,
  })
  await page.route(
    (url) => url.pathname === `/api/session/${coordinatorID}`,
    (route) => {
      if (lineageFailed) return route.fallback()
      lineageFailed = true
      return route.fulfill({
        status: 503,
  ...
  await page.goto(`/${base64Encode(directory)}/session/${nestedChildID}`, { waitUntil: "domcontentloaded" })
  await expect(page.getByRole("heading", { name: nestedChildTitle })).toBeVisible({ timeout: 60_000 })
  ...
  await expect(panel).toContainText(nestedChildTitle)
  await expect(panel).toContainText(siblingTitle)
  await expect(page.locator('[data-slot="session-agents-warning"]')).toHaveCount(0)
  expect(lineageFailed).toBe(true)
  expect(requested).toEqual([rootID])
})

for (const layout of layouts) {
  test(`shows the global active count and only the current session tree in the ${layout.name}`, async ({ page }) => {
    const other = run({
      id: "arun_agents_other_child",
      sessionID: otherChildID,
      callerSessionID: otherRootID,
    ...
    const panel = page.locator('[data-component="agents-panel"]')
    await expect(panel.locator('[data-component="agent-run-row"]')).toHaveCount(1)
    await expect(panel).toContainText(childTitle)
    await expect(panel).not.toContainText(otherChildTitle)
  })
}
...
test("keeps stale agent data visible and retries after a repair failure", async ({ page }) => {
  ...
  await setup(page, {
    ...
    agentRunOverview: () => {
      const current = recovered ? snapshotWithUnknown(rootID) : snapshot(rootID)
      return { body: { nodes: current.nodes, active: current.active, history: current.history } }
    },
  })
  await page.route(
    (url) => url.pathname === "/api/event",
...
function node(sessionID: string, parentSessionID: string, title: string, createdAt: number) {
  return { sessionID, parentSessionID, title, agent: "build", createdAt }
}
