...
Open the **Agents** tab in the existing right workspace to monitor delegated work for the current session. Its badge shows the number of active runs across direct children and nested descendants.

The panel always includes active runs and shows the 10 most recent terminal child sessions by default. Choose **Show history** to reveal older terminal sessions. Expand a row to inspect its details, or choose **Open session** to navigate to its child session.
