...
Open the permanent **Agents** tab in the existing right workspace to monitor delegated work across every session on the connected server. Its badge shows the number of active agent sessions across all session trees.

The panel always includes active runs and shows the 10 most recent terminal child sessions by default. Choose **Show history** to reveal older terminal sessions from the server. Expand a row to inspect its details, or choose **Open session** to navigate to its child session.
...
On mobile, the active-count badge opens the Agents panel in a right-side drawer.
