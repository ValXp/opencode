...
Each row can show an activity summary and, separately, **No activity for N** freshness. A long freshness interval does not establish that a run is stalled; the [AgentRun status](/docs/agents#statuses) remains authoritative. Summaries are local by default; model-generated summaries require explicit opt-in and can use model tokens and incur provider costs.
