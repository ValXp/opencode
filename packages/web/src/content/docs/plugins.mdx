...
`agent.run.updated` carries `{ info: AgentRun.Info }`. It is an advisory live event, not a durable replay feed. Plugins that need a complete view should fetch `GET /api/session/:sessionID/agent-run` initially and after reconnecting, then use the event for live refreshes between snapshots.
