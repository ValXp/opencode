...
| `GET`    | `/api/agent-run`                         | Get global agent-run overview         | Returns `AgentRun.Overview`                                                        |
| `GET`    | `/api/session/:sessionID/agent-run`      | Get the session's agent-run snapshot  | Returns `AgentRun.Snapshot`                                                        |
...
#### Agent-run overviews and snapshots

`GET /api/agent-run` returns an overview across every session tree on the connected server:

```ts
{
  nodes: AgentRun.Node[]
  active: AgentRun.Info[]
  history: AgentRun.Info[]
}
```

`active` includes every durable `running` or `retrying` record. A queued run is represented as `running` without `time.started` and remains active while waiting to begin. More than one active record can refer to the same child Session, so clients should reconcile related records rather than treating `active.length` as an active-agent count.

`history` is newest-first. It contains the 100 most recent terminal runs and can include additional terminal successors required to reconcile the returned active records. `nodes` contains the Session hierarchy needed to place all returned records into a forest.

`GET /api/session/:sessionID/agent-run` returns a snapshot rooted at the requested session:
...
`nodes` describes the nested descendant-session tree. `active` contains runs in `running` or `retrying` state, while `history` contains at most the 100 most recent terminal runs in newest-first order.

Each `AgentRun.Info.id` is a durable execution identity, distinct from its child `sessionID`. A resumed child session has a new run identity and may reference the earlier run through `previousRunID`.

Both responses include the Session nodes needed to place their runs in the hierarchy. Active runs are ordered oldest-first, while terminal history is ordered newest-first.

The optional `activity.summary` describes recent activity. OpenCode derives it locally by default; model-generated summaries are opt-in and can use model tokens and incur provider costs. `activity.at` is a separate freshness timestamp and must not be interpreted as an authoritative stalled signal. See [Agents](/docs/agents#statuses) for lifecycle and restart semantics.
...
The stream can emit `agent.run.updated` with `{ info: AgentRun.Info }`. This event is advisory and is not a durable replay feed. For server-wide monitoring, subscribe to the complete server event stream, fetch `/api/agent-run` initially and after reconnecting, then replay concurrent updates by `AgentRun.Info.version`. Use the Session-scoped snapshot when only one tree is needed.
