...
`nodes` describes the nested descendant-session tree. `active` contains runs in `running` or `retrying` state, while `history` contains all terminal runs in newest-first order. Clients can apply their own display limit.
...
Snapshots include every active run and at most the 100 most recent terminal runs, plus the Session nodes needed to place those runs in their descendant hierarchy.
...
The stream can emit `agent.run.updated` with `{ info: AgentRun.Info }`. This event is advisory and is not a durable replay feed. Fetch the agent-run snapshot initially and after reconnecting; use update events for live refreshes between snapshots.
